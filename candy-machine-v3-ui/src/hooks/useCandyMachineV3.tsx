  ...
  const mint = React.useCallback(
    async (
      ...
      const allowList = opts.groupLabel &&
       ...
       console.log("mintttttttt", {quantityString, opts});
...
// Replace with your Candy Machine public key
const candyMachineId = new PublicKey('4rYtJRQStiMYT2EG7Re5YtcH7tMc2npJN7DLpBwQevjT');

// Fetch account info
const accountInfo = await connection.getAccountInfo(candyMachineId);

// Parse account data
const data = accountInfo.data; // This is a Buffer

// Depending on how your Candy Machine is implemented, the collectionUpdateAuthority might be at a specific offset in the data
// For example, if it's at the beginning of the data:
const collectionUpdateAuthority = new PublicKey(data.slice(0, 32));

console.log('Collection Update Authority:', collectionUpdateAuthority.toBase58());
        ...
        for (let index = 0; index < quantityString; index++) {
          console.log("minttxxxxxtttttt", {candyMachine});
          // console.log({mx.candyMachines().pdas().authority({candyMachine: candyMachine.address})})
          transactionBuilders.push(
            await mintFromCandyMachineBuilder(mx, {
              // candyMachine,
              // user: metaplex.identity().publicKey,
              candyMachine,
              // candyGuard: new PublicKey(candyMachine.candyGuard.address.toBase58(),
              collectionUpdateAuthority: new PublicKey("58TXcVGmxRELaNsT8k5Xw1DpLRsg8xMda7YerLTBC8Hv"),
              // candyMachine.address, // mx.candyMachines().pdas().authority({candyMachine: candyMachine.address})
              group: opts.groupLabel,
              // guards: {

...
const Home = (props: HomeProps) => {
  ...
  const mintButtonClicked = async (label : string) => {
    console.log("mintButtonClicked");
    setIsUserMinting(true);

    metaplex.use(walletAdapterIdentity(wallet!.adapter));
    const nft = await mint(metaplex, candyMachine!, label);
    console.log({ nft });
    if (nft) {
      setNft(nft);
    } else {
      ...
      <Container maxWidth="xs" style={{ position: "relative" }}>
        ...
        >
          ...
          {!connected ? (
            ...
            <>
            <h3>Address : {connected? publicKey?.toBase58() : null}</h3>
              <Grid
                container
                direction="row"
              ...
              >
                ...
                <MintButton onClick={async () => await mintButtonClicked("public")} disabled={isUserMinting || candyMachine?.itemsRemaining === 0}>
                  <Typography variant="body2" color="textSecondary" style={{
                      fontSize: "16px",
                      fontWeight: "bold",
                      color: "white",

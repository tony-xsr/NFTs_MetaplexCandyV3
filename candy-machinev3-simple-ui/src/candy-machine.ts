...
const collectionMintAddress = new PublicKey(candyMachineState.authorityAddress);
const base58Address = collectionMintAddress.toBase58();

console.log(base58Address);
  // console.log(candyMachineState.collectionMintAddress);
  console.log(candyMachineState.authorityAddress);
  try {
    const result: any = await metaplex.candyMachines().mint({
      ...
      // collectionUpdateAuthority: new PublicKey(candyMachineState.mintAuthorityAddresss),
      collectionUpdateAuthority: new PublicKey("3HSt63DeavEkS7G3V8pr6e2cr9Sia1ovooRMzwG1RuaB"),
      ...group && { group },
    });
